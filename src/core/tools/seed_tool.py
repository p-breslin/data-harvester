...
async def discover_urls(config: SeedConfig) -> List[SeededUrl]:
    ...
    log.info(f"[SeedTool] Seeding domain '{domain}' for query: '{config.query}'")
    ...
    try:
        ...
        log.error("[SeedTool] URL seeding timed out")
        return []
    except Exception:
        log.exception("[SeedTool] Unexpected error during seeding")
        return []
    ...
    for entry in top_entries:
        try:
            ...
            log.warning(f"[SeedTool] Skipping invalid entry {entry.get('url')}: {ve}")

    log.info(f"[SeedTool] Returning {len(results)} URLs")
    return results
...
async def seed_tool(
    domain: str, query: str, top_k: int = 10, score_threshold: float = 0.3
) -> str:
    ...
    Args:
        ...
        top_k (int): The maximum number of top-ranked URLs to return.
        score_threshold (float): The minimum relevance score (0.0 to 1.0) for a URL to be considered relevant.

    Returns:
        A JSON string with a "results" array of {url, title, relevance_score, snippet}.
    """
    cfg = SeedConfig(
        domain=domain, query=query, top_k=top_k, score_threshold=score_threshold
    )
